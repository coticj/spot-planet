...
function wpbootstrap_scripts_with_jquery()
{
    ...
    wp_register_script('bootstrap-script', get_template_directory_uri() . '/js/bootstrap.min.js', array(
        'jquery'
    ));
        
    if (is_front_page()) {
		//currently gmaps is only needed on front-page
		wp_register_script('gmaps-script', 'http://maps.google.com/maps/api/js?sensor=true', array(
        'jquery'
		));
		
        wp_register_script('front-page-script', get_template_directory_uri() . '/js/front-page.js', array(
            'jquery'
        ));
...
function myplugin_add_custom_box()
{
    ...
    foreach ($screens as $screen) {
        add_meta_box('coordinates', __('Coordinates', 'myplugin_textdomain'), 'myplugin_inner_custom_box', $screen);
    }
}
...
function myplugin_save_postdata($post_id)
{
    ...
    // either using 
    add_post_meta($post_ID, 'lat', $datalat, true) or update_post_meta($post_ID, 'lat', $datalat);
    add_post_meta($post_ID, 'lng', $datalng, true) or update_post_meta($post_ID, 'lng', $datalng);
